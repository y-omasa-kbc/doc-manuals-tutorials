# **Azure Functions を用いたWeb API作成　CosmosDBとの連携(Azure DocumentDB)**

## **1. はじめに**

このハンズオンでは、前回学習した **Azure Functions** を拡張し、NoSQL データベースサービスである **Azure DocumentDB** と連携する方法を学びます。

Azure DocumentDBは以前Azure CosmosDB for MongoDBであったものが、別の名称で提供されるようになりました。内容はいぜんとしてAzure CosmosDBと同じように扱えます。

今回のハンズオンでは、HTTPリクエストをトリガーとして、Pythonで記述された関数を実行し、Cosmos DBにデータを保存したり、保存されたデータを読み取ったりする一連の流れを体験します。この資料は、現在主流の **v2プログラミングモデル** (function_app.pyを使用) に準拠しています。

## **2. Azure DocumentDB 概論**

Azure DocumentDB（MongoDBとの互換性あり）は、Microsoft Azureが提供するフルマネージド型のNoSQLデータベースサービスである。MongoDBのワイヤープロトコルとAPIに完全互換しており、既存のMongoDBアプリケーションやツールをそのまま利用しながら、Azureのエンタープライズグレードのインフラストラクチャ上で運用できます。

2.1 フルマネージドサービス
- パッチ適用、バックアップ、モニタリングが自動化
- ハードウェアのプロビジョニングや設定が不要
- 99.999%の可用性SLAを提供

2.2 自動スケーリング
- ストレージの自動拡張
- スループットのオンデマンドスケーリング
- サーバーレスオプションによる従量課金モデル

2.3 グローバル分散
- 複数のAzureリージョンにデータをレプリケーション可能
- 単一マスターまたはマルチマスター構成を選択可能
- 地理的に分散したユーザーに対して低レイテンシを実現

2.4 エンタープライズ機能
- 暗号化（保存時、転送時）
- 詳細なアクセス制御（RBAC、IPファイアウォール）
- 監査ログとコンプライアンス対応
- VNet統合とプライベートエンドポイント



## **3. 学習目標**

* Azure DocumentDB のリソースを作成し、基本的な設定ができる。  
* Azure Functions から Azure DocumentDB への接続設定を構成できる。  
* Python と pymongo ライブラリを使用して、Azure DocumentDB のデータを操作する関数を function_app.py に作成できる。  
* HTTPトリガーを利用して、Azure DocumentDB にデータを追加 (Create) および取得 (Read) する Web API を作成できる。
...
### **ステップ1: Azure DocumentDB アカウントの作成**

まず、データの格納先となる Azure DocumentDB アカウントを Azure Portal 上に作成します。

1. [Azure Portal](https://portal.azure.com/) にサインインします。  
2. 画面左上の「**＋リソースの作成**」をクリックします。  
3. 検索ボックスに「Azure Cosmos DB」と入力し、表示された「Azure Cosmos DB」を選択して「**作成**」をクリックします。  
4. API を選択する画面が表示されます。「**Azure DocumentDB (MongoDB との互換性あり)**」の「**作成**」をクリックします。  
5. 次にリソースの種類を選択する画面が表示されます。このハンズオンでは従来のRU(Request Unit)モデルを使用します。「**要求ユニット (RU) データベース　アカウント**」の「**作成**」をクリックしてください。  
6. 以下の情報を入力して、Cosmos DB アカウントを作成します。

| 設定項目 | 説明・入力値 |
| :---- | :---- |
| **ワークロードの種類** | **開発/テスト** を選択します。（一時的利用のため） |
...
| **容量モード** | 「**サーバレス**」を選択します。 |
| **バージョン** | 使用したいバージョンを選択します。 |

7. 入力後、「**レビュー＋作成**」をクリックし、内容を確認してから「**作成**」をクリックします。デプロイが完了するまで数分かかります。
